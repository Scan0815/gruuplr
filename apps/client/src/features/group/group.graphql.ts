...
import { CreateGroupInput, GroupDto, GroupMemberDto, Mutation, Query, UpdateGroupInput } from '../../generated/graphql';
...
export class GroupGraphQL {
  ...
  /**
   * Updates an existing group via GraphQL.
   *
   * @param id The group ID
   * @param name Optional new group name
   * @param description Optional new group description
   * @returns The updated group details
   */
  static async updateGroup(id: string, name?: string, description?: string): Promise<GroupDto> {
    const mutation = gql`
    ...
    const variables = { input: { id, name, description } };
    const result = await this.graphqlService.request<Mutation, { input: UpdateGroupInput }>(
      this.endpoint,
      mutation,
      variables
    );
  ...
  static async deleteGroup(id: string): Promise<boolean> {
    ...
    const variables = { id };
    const result = await this.graphqlService.request<Mutation, { id: string }>(
      this.endpoint,
      mutation,
      variables
    );
